...
class ResNet_50_CCC_T(nn.Module):
    def __init__(self, block=Bottleneck, layers=[3,4,6,3], channels = 64, num_classes=2):
        ...
        self.ViT = ViT(sequence_len = channels*32,c = channels*1,num_patches=8, flag_em=True,flag_mlp=True,dim=512)
        
        self.conv2 = nn.Conv1d(channels, channels*16, kernel_size=1, stride=1, padding=0,bias=False)
        self.conv3 = nn.Conv1d(channels*16, channels*16, kernel_size=3, stride=2, padding=1,bias=False)

        self.avgpool = nn.AdaptiveAvgPool1d(1)
        self.linear1 = nn.Linear(channels * 32, channels * 8)
    ...
    def forward(self, x):
        ...
        x_t1 = self.ViT(x)
        x_t1 = x + x_t1
        x_t1 = self.conv2(x_t1)
        x_t1 = self.conv3(x_t1)
        x_t1 = self.maxpool(x_t1)

        x_c_t = torch.concat([x_c,x_t1],dim=1)
...
if __name__ == '__main__':
    pre_model = ResNet_50_CCC_T(Bottleneck, layers=[3,4,6,3],channels = 16).cuda()

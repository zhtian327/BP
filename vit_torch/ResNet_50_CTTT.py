...
class ResNet_50_CTTT(nn.Module):
    def __init__(self, block=Bottleneck, layers=[3,4,6,3], channels = 64, num_classes=2):
        ...
        self.ViT_1 = ViT(sequence_len = channels*128,c = channels*4,num_patches=8, flag_em=True,flag_mlp=True,dim=256)
        self.ViT_2 = ViT(sequence_len = channels*64,c = channels*4,num_patches=8,flag_em=True,flag_mlp=True,dim=192)
        self.ViT_3 = ViT(sequence_len = channels*32,c = channels*4,num_patches=8, flag_em=True,flag_mlp=True,dim=160)
        self.avgpool = nn.AdaptiveAvgPool1d(1)
        self.linear1 = nn.Linear(channels * 4, channels)
        self.linear2 = nn.Linear(channels, num_classes)
...
if __name__ == '__main__':
    pre_model = ResNet_50_CTTT(Bottleneck, layers=[3,4,6,3],channels = 16).cuda()

...
class ResNet_50_CC_TT(nn.Module):
    def __init__(self, block=Bottleneck, layers=[3,4,6,3], channels = 64, num_classes=2):
        ...
        self.ViT_2 = ViT(sequence_len = channels*32,c = channels*1,num_patches=8,flag_em=True,flag_mlp=True,dim=256)

        self.conv2 = nn.Conv1d(channels, channels*8, kernel_size=1, stride=1, padding=0,bias=False)
        self.conv3 = nn.Conv1d(channels*8, channels*8, kernel_size=3, stride=2, padding=1,bias=False)

        self.avgpool = nn.AdaptiveAvgPool1d(1)
        self.linear1 = nn.Linear(channels * 8 * 2, channels * 8)
    ...
    def forward(self, x):
        ...
        x_t2 = self.conv3(x_t2)

        x_c_t = torch.concat([x_c,x_t2],dim=1)
...
if __name__ == '__main__':
    pre_model = ResNet_50_CC_TT(Bottleneck, layers=[3,4,6,3],channels = 16).cuda()

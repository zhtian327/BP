...
class SELayer(nn.Module):
    def __init__(self, channel, reduction=16):
        ...
        self.fc = nn.Sequential(
            ...
            nn.Conv1d(channel, channel // reduction,kernel_size=1, stride=1, bias=False),
            nn.ReLU(inplace=True),
            #nn.Linear(channel // reduction, channel, bias=False),
            nn.Conv1d(channel // reduction,channel, kernel_size=1, stride=1, bias=False),
            nn.Sigmoid()
        )
    def forward(self, x):
        b, c, _ = x.size()
...
class ResNet_50_C_TTT_SE(nn.Module):
    def __init__(self, block=Bottleneck, layers=[3,4,6,3], channels = 64, num_classes=2):
        ...
        self.ViT_1 = ViT(sequence_len = channels*32,c = channels*1,num_patches=8, flag_em=True,flag_mlp=True,dim=384)
        self.ViT_2 = ViT(sequence_len = channels*32,c = channels*1,num_patches=8,flag_em=True,flag_mlp=True,dim=192)
        self.ViT_3 = ViT(sequence_len = channels*32,c = channels*1,num_patches=8,flag_em=True,flag_mlp=True,dim=128)

        self.conv2 = nn.Conv1d(channels, channels*4, kernel_size=1, stride=1, padding=0,bias=False)
        self.conv3 = nn.Conv1d(channels*4, channels*4, kernel_size=3, stride=1, padding=1,bias=False)
        self.se = SELayer(channels*8)

        self.avgpool = nn.AdaptiveAvgPool1d(1)
        self.linear1 = nn.Linear(channels * 8, channels * 4)
    ...
    def forward(self, x):
        ...
        x_t3 = self.conv2(x_t3)
        x_t3 = self.conv3(x_t3)

        x_c_t = torch.concat([x_c,x_t3],dim=1)
        x_c_t = self.se(x_c_t)

        x_c_t = self.avgpool(x_c_t)
        x_c_t = x_c_t.view(x_c_t.size(0), -1)
        x_c_t = self.linear1(x_c_t)
        out = self.linear2(x_c_t)

        return out


if __name__ == '__main__':
    pre_model = ResNet_50_C_TTT_SE(Bottleneck, layers=[3,4,6,3],channels = 16).cuda()
    summary(pre_model,(1,8*128))
    img = torch.randn(1, 1, 8 * 128)
    out = pre_model(img)
